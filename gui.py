...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.initializeRegisterStudentWidgets()
    
    def initializeRegisterStudentWidgets(self):
        self.cboRegisterStudentInfo = self.findChild(QComboBox, 'cboRegisterStudentInfo')
        self.cboRegisterCourseInfo = self.findChild(QComboBox, 'cboRegisterCourseInfo')

        self.btnRegisterStu = self.findChild(QPushButton, 'btnRegisterStu')
        self.btnRegisterStu.clicked.connect(self.btnRegisterStuClickHandler)

        self.registerStuLbl = self.findChild(QLabel, 'registerStuLbl')

        colNames, rows = getStudentIdsAndNames()
        print(colNames, rows)
        for row in rows:
            self.cboRegisterStudentInfo.addItem(row[1], userData=row[0])
        # self.cboRegisterStudentInfo.currentIndexChanged.connect(self.cboStudentRegistrationCurrentIndexChangedHandler)

        colNames, rows = getCourseIdsAndNames()
        print(colNames, rows)
        for row in rows:
            self.cboRegisterCourseInfo.addItem(row[1], userData=row[0])
        # self.cboRegisterCourseInfo.currentIndexChanged.connect(self.cboCourseRegistrationCurrentIndexChangedHandler)

    def btnRegisterStuClickHandler(self):
        try:
            stuId = self.cboRegisterStudentInfo.currentData()
            courseId = self.cboRegisterCourseInfo.currentData()
            result = registerCourseToStudent(stuId,courseId)
        except Exception as e:
            self.registerStuLbl.setText(str(e))   

        else:     
            if result == 1:
                self.registerStuLbl.setText('Success')
            else:
                self.registerStuLbl.setText('Failure')
    ...
    def refreshAllStudentTab(self):
        colNames, data = getAllStudents()
        self.displayDataInTable(colNames, data, self.tblAllStudents)

    def refreshUpdateStudentTab(self):
        colNames, rows = getStudentIdsAndNames()
        print(colNames, rows)
        self.cboStudentInfo.clear()
    ...
    def refreshAllTabs(self):
        self.refreshAllStudentTab()
        self.refreshUpdateStudentTab()             
