...
def getCourseIdsAndNames():
    sql =f"SELECT course_id, name FROM school.courses;"
    return executeQueryAndReturnResult(sql)    
...
def registerCourseToStudent(sId, cId):
    sql = f"INSERT INTO `school`.`enrollments` (`student_id`, `course_id`) VALUES ('{sId}', '{cId}');"
    return executeQueryAndCommit(sql)
